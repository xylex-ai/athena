...
async fn main() -> std::io::Result<()> {
    ...
    let cache: Arc<Cache<String, Value>> = Arc::new(
        Cache::builder().time_to_live(Duration::from_secs(60)).build()
    );
    ...
    let client: Client = Client::builder()
        ...
        .build()
        .unwrap();
    let app_state: Data<AppState> = Data::new(AppState { cache, immortal_cache, client });
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                async move {
                    ...
                    res.headers_mut().insert(header::SERVER, "XYLEX/0".parse().unwrap());
                    Ok(res)
                }
            })
    ...
    match scylla_list_tables().await {
        ...
        Err(e) => {
            HttpResponse::InternalServerError().json(
                json!({"error": format!("Error listing Scylla tables: {}", e)})
            )
        }
    }
}
...
async fn scylla_query_columns(req: HttpRequest) -> impl Responder {
    if let Some(table_name) = req.query_string().split('=').nth(1) {
        match scylla_list_columns(table_name).await {
            ...
            Err(e) =>
                HttpResponse::InternalServerError().json(
                    json!({"error": format!("Error listing columns for table {}: {}", table_name, e)})
                ),
        }
    } else {
...
async fn scylla_query() -> Result<(), Box<dyn Error>> {
    ...
    session.query_unpaged("INSERT INTO users.users (user_id) VALUES(?)", (to_insert,)).await?;

    // Query rows from the table and print them
    let mut iter = session
        .query_iter("SELECT user_id FROM users.users", &[]).await?
...
async fn scylla_list_tables() -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    ...
    let mut iter = session.query_iter(query, &[]).await?.rows_stream::<(String,)>()?;
...
async fn scylla_list_columns(
    ...
    let query =
        format!("SELECT column_name FROM system_schema.columns WHERE keyspace_name = 'users' AND table_name = '{}'", table_name);
    let mut iter = session.query_iter(query.as_str(), &[]).await?.rows_stream::<(String,)>()?;
...
async fn get_all_tables_and_columns() -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    ...
    let mut tables_iter = session.query_iter(tables_query, &[]).await?.rows_stream::<(String,)>()?;

    let mut data = Vec::new();

    while let Some((table_name,)) = tables_iter.try_next().await? {
        let columns_query =
            format!("SELECT column_name, type FROM system_schema.columns WHERE keyspace_name = 'users' AND table_name = '{}'", table_name);
        let mut columns_iter: scylla::client::pager::TypedRowStream<(String, String)> = session
            .query_iter(columns_query.as_str(), &[]).await?
